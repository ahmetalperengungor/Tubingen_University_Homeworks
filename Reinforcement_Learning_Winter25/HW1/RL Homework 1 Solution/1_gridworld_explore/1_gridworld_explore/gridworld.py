...
def runEpisode(agent, environment, discount, decision, display, message, pause, episode):
  ...
  while True:
    ...
    if len(actions) == 0:
      message("EPISODE "+str(episode)+" COMPLETE: RETURN WAS "+str(g)+"\n")
      return g  # TODO: IMPLEMENT "return" obtained in the episode
...
if __name__ == '__main__':
  ...
  for episode in range(1, opts.episodes+1):
    g = runEpisode(a, env, opts.discount, decisionCallback, displayCallback, messageCallback, pauseCallback, episode)
    episode_returns.append(g)
  if opts.episodes > 0:
    mean_return = np.mean(episode_returns)
    std_return = np.std(episode_returns)
    print(f"\nAfter {opts.episodes} episodes:")
